...
class patternIdentification:
    def __init__(self, queryid1, queryid2,n_clusters=4, anomaly_contamination=0.02):
        ...
        qry = self.get_query()
        self.df = self.process_data(qry)
        
        
    def get_query(self):
        try:
            qry = "SELECT QUERY FROM APPLICATIONQUERIES WHERE ID = ?"
            if self.queryid1 and not self.queryid2:
                results = sql.execute_Sql(qry, (self.queryid1))
            else:
                results = sql.execute_Sql(qry, (self.queryid2))
                
            if results.empty:
                raise ValueError(f"No query found with ID ") 
    ...
    def process_data(self,qry):
        print("Processing data with query:", qry)
        A = sql.execute_Sql(qry)
        print(A.head())
        ...
        weekly = (
            A.set_index("EVENTDATE")
                .groupby("CARDNUMBER")
                .resample("W")["DEVICEID"].count()
                .unstack(fill_value=0)
        )
        if weekly.shape[1] >= 2:
            weekly["week_delta"] = weekly.iloc[:, -1] - weekly.iloc[:, -2]  # type: ignore
        else:
        ...
        A_feat.rename(columns={"A_CARDNUMBER": "CARDNUMBER",
                                "A_LOCATION": "LOCATION"}, inplace=True)

        B = sql.execute_Sql(qry)
        print(B.head())
        ...
        merged = (
            A_feat
                .merge(B_feat, on="CARDNUMBER", how="inner")
                .merge(weekly, on="CARDNUMBER", how="left")
        )

        # Continuity metrics
        merged["event_diff"] = (
            merged["A_event_count"] - merged["B_prev_event_count"]
        )
        merged["location_switch"] = (
            (merged["LOCATION"] != merged["B_prev_location"]).astype(int)
        )

        # Replace NaNs produced by missing prev_* columns with 0
        merged[["event_diff", "location_switch"]] = merged[["event_diff",
                                                            "location_switch"]].fillna(0)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process clustering and anomaly detection on event data.")
    parser.add_argument("--queryid1", type=str, required=True, help="Query ID 1 to fetch data")
    parser.add_argument("--queryid2", type=str, required=True, help="Query ID 2 to fetch data")
    parser.add_argument("--n_clusters", type=int, default=4, help="Number of KMeans clusters (default: 4)")
    parser.add_argument("--anomaly_contamination", type=float, default=0.02, help="Isolation Forest contamination ratio (default: 0.02)")
    args = parser.parse_args()
    pattern_identification = patternIdentification(args.queryid1, args.queryid2, args.n_clusters, args.anomaly_contamination)
