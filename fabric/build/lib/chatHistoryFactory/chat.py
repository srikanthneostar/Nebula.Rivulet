...
class ChatHistoryFactory:
    ...
    def create_chat_history(
        history_type: ChatHistoryType, session_id: str
    ) -> ChatMessageHistory:
        if history_type == ChatHistoryType.FILE:
            return FileChatHistory(session_id)
        elif history_type == ChatHistoryType.REDIS:
            return RedisChatHistory(session_id)
        else:
            raise ValueError(
                f"Invalid history type: {history_type}. Expected one of {list(ChatHistoryType)}"
            )
